...
- 支持将同步文件直接拖拽到接收区域，避免打开系统文件窗口时浏览器窗口重新定位。
- 点击选择文件会打开同一浏览器窗口内的书签桥侧边栏，再回到 Popup 自动载入；不需要独立标签页，也避免从悬浮 Popup 直接唤起 macOS 文件面板。
- 支持本次弹窗会话的多次合并历史，可分别撤销每次合并；大书签树处理过程中可以取消。
- 对同步内容进行 gzip 压缩，并在加密、解密和合并时显示处理进度。
- 图标使用完整圆角方形画布与高对比桥形标识，在工具栏和扩展管理页保持清晰可见。
